...
async function getPgPool() {
  ...
  if (isNeon) {
    // Use Neon's HTTP-based SQL function (no WebSocket/ws needed, works with Next.js webpack)
    const { neon } = await import('@neondatabase/serverless');
    const sql = neon(connectionString);

    // Create a Pool-like wrapper around the HTTP neon() function
    // so all existing pgPool.query(...) calls work unchanged.
    // Use sql.query() for conventional parameterized calls (not tagged-template).
    pgPool = {
  ...
  // Ensure tables exist — individual statements for Neon compatibility
  await pgPool.query(`
