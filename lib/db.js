...
export async function getCourses() {
  ...
  if (dialect === 'postgres') {
    ...
    return res.rows.map(row => ({
      ...row,
      SeatsLeft: Math.max(0, (row.Capacity || 30) - (row.EnrolledCount || 0))
    }));
  }
  ...
  return data.Courses.map(course => {
    const enrolled = data.Registrations.filter(r => r.CourseID === course.CourseID).length;
...
export async function getStudents() {
  ...
  return data.Students.map(student => {
    const count = data.Registrations.filter(r => r.StudentID === student.StudentID).length;
    return {
      ...student,
      EnrolledCount: count
    };
...
export async function addStudent(student) {
  ...
  if (dialect === 'postgres') {
    const pool = await getPgPool();
    if (!StudentID) {
      const resCount = await pool.query('SELECT COUNT(*) FROM Students');
      const nextNum = parseInt(resCount.rows[0].count, 10) + 1001;
  ...
  if (dialect === 'mysql') {
    ...
    if (!StudentID) {
      ...
      const nextNum = Number(countRes[0].cnt) + 1001;
      StudentID = `STU-${nextNum}`;
    } else {
      StudentID = StudentID.trim().toUpperCase();
    }
  ...
  const data = readLocalData();
  if (data.Students.some(s => s.Email.toLowerCase() === Email)) {
    throw new Error(`Student with email "${Email}" is already registered.`);
  }

  if (!StudentID) {
    const nextNum = data.Students.length + 1001;
    StudentID = `STU-${nextNum}`;
  } else {
    StudentID = StudentID.trim().toUpperCase();
...
export async function getRegistrations() {
  ...
  const data = readLocalData();
  const studentMap = new Map(data.Students.map(s => [s.StudentID, s]));
  const courseMap = new Map(data.Courses.map(c => [c.CourseID, c]));

  return data.Registrations
    .map(r => {
      const student = studentMap.get(r.StudentID) || { Name: 'Unknown', Email: '' };
      const course = courseMap.get(r.CourseID) || { CourseName: 'Unknown', Duration: '', Department: '' };
      return {
